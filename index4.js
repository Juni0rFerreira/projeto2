...
// 4. Criar uma lista de hobbies únicos
const hobbiesUnicos = [];

for (var i = 0; i < pessoas.length; i++) {
    for (var j = 0; j < pessoas[i].hobbies.length; j++) {
        const hobbie = pessoas[i].hobbies[j];
        if (!hobbiesUnicos.includes(hobbie)) {
            hobbiesUnicos.push(hobbie);
        }
    }
}

console.log(hobbiesUnicos);